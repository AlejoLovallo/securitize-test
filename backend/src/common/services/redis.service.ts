...
import { LoggerService, LoggerServiceKey } from "@interval-network/nest-common";
import { createClient } from "redis";
import {
  ErrorCode,
  ErrorWithData,
  parseEnvVar,
} from "@interval-network/common";
import { MAGIC_PROXY_PARTNER_CONFIG_PREFIX } from "@interval-network/common";
...
export class RedisService implements OnModuleInit {
  ...
  constructor(
    @Inject(LoggerServiceKey) private readonly loggerService: LoggerService
  ) {
  ...
  async setValue(key: string, value: any, ex?: number) {
    await this.redisClient.set(
      `${MAGIC_PROXY_PARTNER_CONFIG_PREFIX}-${key}`,
      value,
      {
        EX: ex
          ? ex
          : (parseEnvVar(process.env)(
              NumberFromString,
              "REDIS_DEFAULT_EXPIRATION_TIME"
            ) ?? 9000000000000),
      }
    );
  }
   ...
   * @returns {any}
   */
  private async fetchValueOrFail(
    key: string,
    throwError?: boolean
  ): Promise<any> {
    ...
    if (ttl === this.REDIS_KEY_NOT_EXISTS && throwError) {
      throw new ErrorWithData(
        "Key does not exists or has expired",
